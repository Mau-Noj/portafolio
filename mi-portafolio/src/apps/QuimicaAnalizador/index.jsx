...
const FormulaLibro = ({ formula, className = '' }) => {
  ...
  while (i < formula.length) {
    let letters = '';
    while (i < formula.length && !/[0-9]/.test(formula[i])) letters += formula[i++];
    if (letters) parts.push(<span key={`l${i}`}>{letters}</span>);
    let digits = '';
    while (i < formula.length && /[0-9]/.test(formula[i])) digits += formula[i++];
    if (digits) parts.push(<sub key={`d${i}`} style={{ fontSize: '0.72em', lineHeight: 0 }}>{digits}</sub>);
  }
...
const TECLADO_ROWS = [
  {
    ...
    items: ELEMENTOS_COMUNES.map(e => ({ label: e, value: e, tipo: 'elemento' })),
  },
  {
    ...
    items: [
      ...
      { label: ')', value: ')', tipo: 'grupo' },
      ...['2','3','4','5','6','7','8','9'].map(n => ({ label: n, value: n, tipo: 'numero' })),
...
const TecladoQuimico = ({ onInsert, onDelete, onClear }) => {
  ...
  return (
    <div className="qa__teclado">
      <button className="qa__teclado-toggle" onClick={() => setExpandido(e => !e)}>
        <span>⌨ Teclado químico</span>
        <span style={{ fontSize: '0.7rem', opacity: 0.6 }}>{expandido ? '▲ ocultar' : '▼ mostrar'}</span>
      </button>
      {expandido && (
...
const SeccionAnalizador = ({ api }) => {
  ...
  const insertarTexto = (texto) => {
    ...
    if (!input) { setFormula(f => f + texto); return; }
  ...
  const borrarUltimo = () => {
    const input = inputRef.current;
    if (!input) { setFormula(f => f.slice(0, -1)); return; }
    const start = input.selectionStart;
    if (start === 0) return;
  ...
  const analizar = async (f) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.detalle || data.error || 'Error al analizar');
      setResultado(data);
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <section className="qa__section">
      <div className="qa__section-header">
        ...
        <p className="qa__section-sub">Obtén los tres sistemas de nomenclatura química al instante.</p>
      </div>

      <div className="qa__chips">
        {EJEMPLOS.map(({ f, label }) => (
          <button key={f} className="qa__chip"
            onClick={() => { setFormula(f); analizar(f); }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {formula && (
        <div className="qa__formula-preview">
          ...
          <FormulaLibro formula={formula} className="qa__formula-preview-value" />
        </div>
      )}
      ...
      <div className="qa__input-row">
        <input
          ...
          autoComplete="off"
        />
        <button className="qa__btn-primary" onClick={() => analizar()} disabled={loading}>
      ...
      <TecladoQuimico
        ...
        onClear={() => { setFormula(''); setResultado(null); setError(''); }}
      />

      {error && <div className="qa__error-box" style={{ marginTop: '16px' }}>⚠ {error}</div>}

      {resultado && (
        <div className="qa__resultado" style={{ marginTop: '24px' }}>
          <div className="qa__result-header">
            <FormulaLibro formula={resultado.formulaEntrada} className="qa__formula-display" />
            <span className="qa__badge qa__badge--info">
              {resultado.tipoCompuesto?.replace(/_/g, ' ')}
            </span>
            <span className={`qa__badge ${resultado.valida ? 'qa__badge--ok' : 'qa__badge--err'}`}>
              {resultado.valida ? '✓ Válida' : '✗ Inválida'}
            </span>
          </div>

          {resultado.usoComun && <p className="qa__uso">{resultado.usoComun}</p>}
            ...
            <div className="qa__elementos">
              {resultado.elementos.map(el => (
                <div className="qa__elemento" key={el.simbolo}>
                  <FormulaLibro
                    formula={el.simbolo + (el.subindice > 1 ? String(el.subindice) : '')}
          ...
          {resultado.pasos?.length > 0 && (
            <details className="qa__pasos">
              <summary className="qa__pasos-title">Ver razonamiento paso a paso</summary>
              <div className="qa__pasos-body">
                {resultado.pasos.map((p, i) => (
                  <p className="qa__paso" key={i}>{p}</p>
                ))}
              </div>
            </details>
          )}
  ...
  return (
    <section className="qa__section">
      <div className="qa__section-header">
        ...
        <p className="qa__section-sub">Detecta errores sintácticos con el parser ANTLR4.</p>
      </div>

      <div className="qa__chips">
        {EJEMPLOS_VAL.map(e => (
          <button key={e} className="qa__chip"
            onClick={() => { setFormula(e); validar(e); }}>
            {e}
          </button>
        ))}
      </div>
      ...
      <div className="qa__input-row">
        <input
          ...
          spellCheck={false}
        />
        <button className="qa__btn-primary" onClick={() => validar()} disabled={loading}>
          {loading ? <span className="qa__spinner" /> : 'Validar'}
        </button>
      </div>

      {error && <div className="qa__error-box">⚠ {error}</div>}

      {resultado && (
        <div className="qa__resultado">
          <div className="qa__result-header">
            <FormulaLibro formula={resultado.formula} className="qa__formula-display" />
            <span className={`qa__badge ${resultado.valida ? 'qa__badge--ok' : 'qa__badge--err'}`}>
          ...
          {resultado.errores?.length > 0 && (
            <div className="qa__errores-list">
              {resultado.errores.map((e, i) => (
                <div className="qa__error-box" key={i}>{e}</div>
              ))}
            </div>
          )}
          ...
          {resultado.valida && (
            <p className="qa__info-box">
              ✓ Sintaxis correcta. Usa el <strong>Analizador</strong> para obtener la nomenclatura completa.
            </p>
          )}
        </div>
      )}
...
const SeccionQuiz = ({ api }) => {
  ...
  useEffect(() => { cargarPregunta(); }, []);

  const cargarPregunta = async () => {
    setLoading(true); setSeleccion(null); setFeedback(null); setError('');
    try {
  ...
  const getClase = (opcion) => {
    ...
    return feedback.correcto ? 'qa__opcion--correcta' : 'qa__opcion--incorrecta';
  };

  const porcentaje = score.total > 0
    ? Math.round((score.correctas / score.total) * 100)
    : null;
  ...
  return (
    <section className="qa__section">
      <div className="qa__section-header">
        ...
        <p className="qa__section-sub">Pon a prueba tu conocimiento de fórmulas y nombres.</p>
      </div>

      <div className="qa__score-bar">
        <div>
          <span className="qa__score-num">{score.correctas}/{score.total}</span>
          <span className="qa__score-label">correctas</span>
        </div>
        {porcentaje !== null && (
          <span className={`qa__badge ${porcentaje >= 70 ? 'qa__badge--ok' : 'qa__badge--err'}`}>
            {porcentaje}%
          </span>
        )}
      </div>
      ...
      {loading && <div className="qa__loading-wrap"><span className="qa__spinner" /></div>}

      {pregunta && !loading && (
        <div className="qa__quiz-card">
          <p className="qa__quiz-pregunta">
            {pregunta.pregunta.split(/(\b[A-Z][a-zA-Z0-9()]*\b)/).map((part, i) =>
          ...
          {feedback && (
            <div className="qa__feedback">
              <p className={feedback.correcto ? 'qa__info-box' : 'qa__error-box'}>
                {feedback.correcto
                  ? `✓ ¡Correcto! ${feedback.explicacion}`
                  : `✗ Incorrecto. La respuesta es: ${feedback.respuestaCorrecta}`}
              </p>
              <button className="qa__btn-primary qa__btn-siguiente" onClick={cargarPregunta}>
                Siguiente →
              </button>
            </div>
          )}
...
const MoleculaBalanceada = ({ formula, coeficiente, lado }) => {
  const colorBase = lado === 'reactivo' ? 'var(--qa-accent2)' : 'var(--qa-accent)';
  return (
  ...
  <table className="qa__tabla qa__tabla-balance">
    <thead>
      <tr>
        <th>Elemento</th>
        {moleculas.map((m, i) => (
          <th key={i} style={{ color: m.lado === 'reactivo' ? 'var(--qa-accent2)' : 'var(--qa-accent)' }}>
            <FormulaLibro formula={m.formula} />
            {m.coeficiente > 1 && <span style={{ fontSize: '0.65rem', marginLeft: 2 }}>×{m.coeficiente}</span>}
          </th>
        ))}
    ...
    <tbody>
      {elementos.map(elem => {
        const sumaR = moleculas.filter(m => m.lado === 'reactivo').reduce((s, m) => s + m.coeficiente * (m.atomos[elem] || 0), 0);
        const sumaP = moleculas.filter(m => m.lado === 'producto' ).reduce((s, m) => s + m.coeficiente * (m.atomos[elem] || 0), 0);
        return (
          <tr key={elem}>
            <td className="qa__tabla-simbolo"><FormulaLibro formula={elem} /></td>
            {moleculas.map((m, i) => (
              <td key={i} style={{ opacity: (m.atomos[elem] || 0) === 0 ? 0.25 : 1 }}>
                {m.coeficiente * (m.atomos[elem] || 0)}
              </td>
            ))}
            <td style={{ fontWeight: 600, color: 'var(--qa-accent2)' }}>{sumaR}</td>
            <td style={{ fontWeight: 600, color: 'var(--qa-accent)'  }}>{sumaP}</td>
            <td>{sumaR === sumaP ? '✓' : '✗'}</td>
          </tr>
        );
      })}
...
const TECLADO_BAL_ROWS = [
  {
    label: 'Elementos frecuentes',
    items: ['H','O','C','N','Na','K','Ca','Fe','Al','Mg','S','P','Cl','Cu','Zn']
...
const SeccionBalanceador = ({ api }) => {
  ...
  const [loading,     setLoading]     = useState(false);
  const [error,       setError]       = useState('');
  // ✅ FIX Bug 2: estado React para el toggle del teclado
  const [tecladoOpen, setTecladoOpen] = useState(true);
  const inputRef = useRef(null);

  const insertarTexto = (texto) => {
    const input = inputRef.current;
    if (!input) { setEcuacion(e => e + texto); return; }
    const start = input.selectionStart;
    const end   = input.selectionEnd;
    const nueva = ecuacion.slice(0, start) + texto + ecuacion.slice(end);
    setEcuacion(nueva);
    setTimeout(() => { input.focus(); input.setSelectionRange(start + texto.length, start + texto.length); }, 0);
  };

  const borrarUltimo = () => {
    const input = inputRef.current;
    if (!input) { setEcuacion(e => e.slice(0, -1)); return; }
    const start = input.selectionStart;
    if (start === 0) return;
  ...
  return (
    <section className="qa__section">
      <div className="qa__section-header">
        ...
        <p className="qa__section-sub">Obtén los coeficientes mínimos enteros por álgebra lineal.</p>
      </div>

      <div className="qa__info-box" style={{ marginBottom: 16, fontSize: '0.8rem' }}>
        Formato: <code style={{ fontFamily: 'var(--qa-mono)', color: 'var(--qa-accent)' }}>H2 + O2 -&gt; H2O</code>
        &nbsp;·&nbsp; Usa <code style={{ fontFamily: 'var(--qa-mono)' }}>-&gt;</code> como flecha. Separa con <code style={{ fontFamily: 'var(--qa-mono)' }}>+</code>.
      </div>

      <div className="qa__chips">
        {EJEMPLOS_BAL.map(({ e, label }) => (
          <button key={e} className="qa__chip" onClick={() => { setEcuacion(e); balancear(e); }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {ecuacion && (
        <div className="qa__formula-preview">
          <span className="qa__formula-preview-label">Vista previa</span>
          <span className="qa__formula-preview-value" style={{ fontSize: '1.1rem' }}>
            {ecuacion.split(/(\s*\+\s*|\s*->\s*|\s*→\s*)/).map((part, i) => {
              const t = part.trim();
              if (t === '+')  return <span key={i} style={{ color: 'var(--qa-text2)', margin: '0 4px' }}>+</span>;
              if (t === '->' || t === '→') return <span key={i} style={{ color: 'var(--qa-text2)', margin: '0 4px' }}>→</span>;
              return part.trim() ? <FormulaLibro key={i} formula={part.trim()} /> : null;
            })}
          </span>
        </div>
      )}
      ...
      <div className="qa__input-row">
        <input
          ...
          autoComplete="off"
        />
        <button className="qa__btn-primary" onClick={() => balancear()} disabled={loading}>
          {loading ? <span className="qa__spinner" /> : 'Balancear'}
        </button>
      </div>

      {/* ✅ FIX Bug 2: toggle con estado React, igual que TecladoQuimico */}
      <div className="qa__teclado">
        <button className="qa__teclado-toggle" onClick={() => setTecladoOpen(v => !v)}>
          <span>⌨ Teclado de ecuaciones</span>
          <span style={{ fontSize: '0.7rem', opacity: 0.6 }}>{tecladoOpen ? '▲ ocultar' : '▼ mostrar'}</span>
        </button>
        {tecladoOpen && (
          <div className="qa__teclado-body">
            {TECLADO_BAL_ROWS.map(row => (
              <div key={row.label} className="qa__teclado-row">
                ...
                <div className="qa__teclado-btns">
                  {row.items.map((item, idx) => (
                    <button key={idx} className={`qa__key qa__key--${item.tipo}`}
                      onClick={() => {
                        if (item.value === 'DEL') borrarUltimo();
                        else if (item.value === 'CLR') { setEcuacion(''); setResultado(null); setError(''); }
      ...
      {error && <div className="qa__error-box" style={{ marginTop: 16 }}>⚠ {error}</div>}

      {resultado && (
        <div className="qa__resultado" style={{ marginTop: 24 }}>
          <div className="qa__result-header" style={{ flexWrap: 'wrap', gap: 8 }}>
            <div className="qa__ecuacion-balanceada">
              {resultado.moleculas.map((m, i) => {
                const totalReact = resultado.moleculas.filter(x => x.lado === 'reactivo').length;
                return (
                  <React.Fragment key={i}>
                    {i > 0 && (
                      <span className="qa__bal-op">{i === totalReact ? '→' : '+'}</span>
                    )}
                    <MoleculaBalanceada formula={m.formula} coeficiente={m.coeficiente} lado={m.lado} />
                  </React.Fragment>
                );
              })}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button className="qa__btn-copy"
              onClick={() => navigator.clipboard?.writeText(resultado.ecuacionBalanceada)}>
              ⎘ copiar
            </button>
          </div>

          <details className="qa__pasos" open>
            <summary className="qa__pasos-title">Tabla de verificación de balance</summary>
            <div style={{ overflowX: 'auto', padding: '8px 16px 16px' }}>
              <TablaBalance moleculas={resultado.moleculas} elementos={resultado.elementosBalanceados} />
            </div>
          </details>
        </div>
      )}
