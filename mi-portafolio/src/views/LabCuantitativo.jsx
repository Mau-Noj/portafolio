...
const SectionExamples = ({ tool }) => (
  <div className="lab-section">
    ...
    {tool.examples.map((ex, i) => (
      <div key={i} className="lab-example" style={{ '--mc': tool.categoryColor }}>
...
const SectionCalculator = ({ tool }) => {
  ...
  const handleCompute = async () => {
    ...
    try {
      ...
      setError('No se pudo conectar con el servidor. Verifica que el backend esté corriendo en localhost:8000');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="lab-section">
      ...
      <p className="lab-calc-note">
        Los cálculos se procesan en el servidor — resultados más precisos y con tabla de iteraciones.
      </p>
  ...
  <div className="lab-section">
    ...
    <div className="lab-apps-grid">
      {tool.applications.map((app, i) => (
        <div key={i} className="lab-app-card" style={{ '--mc': tool.categoryColor }}>
...
const LabModal = ({ tool, onClose }) => {
  ...
  useEffect(() => { setCurrentTool(tool); setTab('explain'); }, [tool]);
    ...
    <div className="lab-fs-overlay" onClick={onClose}>
      <div className="lab-fs-modal" onClick={e => e.stopPropagation()}>
        ...
        <aside className="lab-fs-sidebar" style={{ '--mc': t.categoryColor }}>
          <div className="lab-fs-sidebar-hero">
            ...
            <div className="lab-fs-tags">
              {t.tags.map(tag => (
                <span key={tag} className="lab-fs-tag">{tag}</span>
              ))}
            </div>
          </div>
          ...
          <button className="lab-fs-close" onClick={onClose}>✕ Cerrar</button>
        </aside>
        ...
        <main className="lab-fs-content">
          ...
          {tab === 'related'  && <SectionRelations    tool={t} onNavigate={(nt) => { setCurrentTool(nt); setTab('explain'); }} />}
...
const LabCard = ({ tool, onOpen }) => (
  <div className="lab-card" style={{ '--mc': tool.categoryColor }} onClick={() => onOpen(tool)}>
    ...
    <div className="lab-card-tags">
      {tool.tags.map(t => <span key={t} className="lab-card-tag">{t}</span>)}
    </div>
...
/* ══════════════════════════════════════════════════════════
   MAIN
══════════════════════════════════════════════════════════ */
const CATEGORIES = ['Todos', 'Ing. Económica', 'Inv. Operaciones', 'Estadística', 'Métodos Numéricos'];

export const LabCuantitativo = () => {
  const [active, setActive] = useState('Todos');
  const [modal,  setModal]  = useState(null);

  const filtered = active === 'Todos' ? LAB_TOOLS : LAB_TOOLS.filter(t => t.category === active);
    ...
    <div className="lab-wrap">
      <div className="lab-hero">
        ...
        <h1 className="lab-heading">
          Herramientas<br /><em>matemáticas</em><br />interactivas
        </h1>
        <p className="lab-sub">
          Calculadoras + artículos para Ingeniería Económica,
          Investigación de Operaciones, Estadística y Métodos Numéricos.
        </p>
      </div>
