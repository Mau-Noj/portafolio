...
const SERVICE_ID  = 'service_mpg0wru';
const TEMPLATE_ID = 'REMOVED';
const PUBLIC_KEY  = 'REMOVED';

const SOCIAL_LINKS = [
  { label: 'GitHub',    handle: '@Mau-Noj',             href: 'https://github.com/Mau-Noj',          icon: '⌥' },
  { label: 'YouTube',   handle: '@programandoconpepito', href: 'https://www.youtube.com/@programandoconpepito', icon: '▶' },
  { label: 'LinkedIn',  handle: 'brandon-noj',           href: 'https://www.linkedin.com/in/brandon-mauricio-noj-romero-38b4701b6/', icon: 'in' },
  { label: 'Instagram', handle: '@superpepitopro64',     href: 'https://www.instagram.com/superpepitopro64/', icon: '◎' },
];
...
export const ContactSection = () => {
  ...
  const validate = () => {
    ...
    else if (fields.message.trim().length < 20)       e.message = 'Mínimo 20 caracteres.';
    return e;
  };
  ...
  const resetForm = () => { setStatus('idle'); setErrors({}); };
      ...
      <div className="ct">
        ...
        <aside className="ct__aside">
          ...
          <p className="ct__desc">
            Consultoría técnica, revisión de arquitectura,
            colaboración en proyectos o simplemente una pregunta.
          ...
          <div className="ct__socials">
            {SOCIAL_LINKS.map(s => (
              <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="ct__social">
          ...
          {status === 'success' && (
            <div className="ct__feedback ct__feedback--ok">
              ...
              <div>
                ...
                <p className="ct__fb-sub">Gracias por escribirme. Te respondo en menos de 48 horas a <strong>{fields.email || 'tu email'}</strong>.</p>
              </div>
              <button className="ct__fb-btn" onClick={resetForm}>Enviar otro</button>
            </div>
          )}
            ...
            <div className="ct__feedback ct__feedback--err">
              ...
              <div>
                ...
                <p className="ct__fb-sub">
                  ...
                  <a href="mailto:contacto@mauricionoj.com">contacto@mauricionoj.com</a>
                </p>
              </div>
              <button className="ct__fb-btn" onClick={resetForm}>Reintentar</button>
            </div>
          )}

          {/* ── Formulario ── */}
          {(status === 'idle' || status === 'sending') && (
            <form ref={formRef} className="ct__form" onSubmit={handleSubmit} noValidate>

              {/* Fila nombre + email */}
              <div className="ct__row">
                <div className={`ct__field ${errors.name ? 'ct__field--err' : ''}`}>
                  <label className="ct__label" htmlFor="ct-name">Nombre</label>
                  <input
                    id="ct-name"
                    className="ct__input"
                    type="text"
                    ...
                    autoComplete="name"
                  />
                  {errors.name && <span className="ct__err">{errors.name}</span>}
                </div>

                <div className={`ct__field ${errors.email ? 'ct__field--err' : ''}`}>
                  ...
                  <input
                    ...
                    autoComplete="email"
                  />
                  {errors.email && <span className="ct__err">{errors.email}</span>}
                </div>
              </div>

              {/* Asunto */}
              <div className={`ct__field ${errors.subject ? 'ct__field--err' : ''}`}>
                <label className="ct__label" htmlFor="ct-subject">Asunto</label>
                <select
                  id="ct-subject"
                ...
                {errors.subject && <span className="ct__err">{errors.subject}</span>}
              </div>

              {/* Mensaje */}
              <div className={`ct__field ${errors.message ? 'ct__field--err' : ''}`}>
                <label className="ct__label" htmlFor="ct-message">
                  Mensaje
                  <span className={`ct__char ${charCount > 800 ? 'ct__char--warn' : ''}`}>
                    {charCount}/1000
                  </span>
                </label>
                <textarea
                  ...
                  disabled={status === 'sending'}
                />
                {errors.message && <span className="ct__err">{errors.message}</span>}
              </div>
              ...
              <p className="ct__note">
                Enviado desde <strong>contacto@mauricionoj.com</strong> vía Brevo · 300 emails/día
              </p>
            </form>
          )}
