...
const ARTICULOS = [
  {
    ...
    vol: "01",
    resumen:
      "La Arquitectura de la Información no vive en las pantallas. Vive en las decisiones que preceden a cada píxel.",
    lecturaMin: 8,
  ...
  },
  {
    ...
    vol: "02",
    resumen:
      "Cómo un sistema de diseño deja de ser una librería de componentes y se convierte en lenguaje compartido.",
    lecturaMin: 10,
  ...
  },
  {
    ...
    vol: "03",
    resumen:
      "En banca, la regulación no es un obstáculo al diseño: es una capa de la arquitectura que hay que integrar.",
    lecturaMin: 12,
    tags: ["Fintech", "KYC", "AI"],
    sello: "FX",
    accent: "#0891b2",
  ...
  },
  {
    ...
    vol: "04",
    resumen:
      "Card sorting, tree testing, entrevistas contextuales. Qué método usar, cuándo y cómo evitar sesgos.",
    lecturaMin: 9,
  ...
  },
  {
    ...
    vol: "05",
    resumen:
      "El flujo feliz oculta la mayoría del trabajo real. Diseñar para los errores es donde se decide la calidad.",
    lecturaMin: 7,
  ...
  },
  {
    ...
    vol: "06",
    resumen:
      "WCAG no es una lista de requisitos técnicos. Es un marco para entender que el diseño accesible es siempre mejor diseño.",
    lecturaMin: 11,
...
function EnvelopeCard({ art }) {
        ...
        <div className="env-letter">
          <div className="env-letter-paper">
            ...
            <div className="env-letter-tags">
              {tags.map((t) => (
                <span key={t} className="env-letter-tag">
                  {t}
                </span>
              ))}
            </div>
            ...
            <div className="env-letter-footer">
              {publicado ? (
                <span className="env-letter-cta">
                  Abrir carta <span className="env-letter-arrow">→</span>
                </span>
              ) : (
  ...
  if (publicado) {
    return (
      <Link to={`/articulos/${slug}`} className="env-link">
        {card}
      </Link>
    );
  }
...
function useFadeIn() {
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      (entries) =>
        entries.forEach(
          (e) => e.isIntersecting && e.target.classList.add("env-link--in"),
        ),
    ...
    <section className="articulos-section">
      <header className="articulos-header">
        ...
        <p className="articulos-subtitle">
          Una colección de escritos sobre diseño de experiencias, sistemas de
          información y la disciplina invisible que los une.
          <br />
          Cada carta, un argumento.
        </p>
        <div className="articulos-counter">
          <span className="articulos-counter-num">0{pub}</span>
          <span className="articulos-counter-label">
            publicado{pub !== 1 ? "s" : ""}
          </span>
      ...
      <div className="articulos-grid">
        {ARTICULOS.map((art) => (
          <EnvelopeCard key={art.slug} art={art} />
        ))}
      </div>
    </section>
  );
