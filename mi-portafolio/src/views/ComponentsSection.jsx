...
export const ComponentsSection = () => {
  ...
  const filtrados = useMemo(() => {
    let list = [...todos];
    if (catActiva !== 'all') list = list.filter(c => c.categoria === catActiva);
    if (query.trim()) {
    ...
    if (orden === 'carbono')  list.sort((a, b) => a.huella_carbono - b.huella_carbono);
    if (orden === 'toxicidad') {
      ...
      <div className="cs">
        ...
        <header className="cs__header">
          ...
          <p className="cs__subtitle">
            Fichas técnicas con perspectiva dual — ingeniería electrónica
            e impacto ambiental integrados en cada componente.
          </p>
          ...
          <div className="cs__stats">
            ...
            <div className="cs__stat">
              <span className="cs__stat-n">
                {(todos.reduce((a, c) => a + c.huella_carbono, 0) / todos.length).toFixed(1)}g
              </span>
        ...
        <div className="cs__controls">
          ...
          <div className="cs__search">
            ...
            />
            {query && (
              <button className="cs__search-clear" onClick={() => setQuery('')}>✕</button>
            )}
          </div>

          {/* Orden */}
          <select className="cs__sort" value={orden} onChange={e => setOrden(e.target.value)}>
        ...
        {filtrados.length === 0 ? (
          <div className="cs__empty">
            <p>No se encontraron componentes para "<strong>{query}</strong>"</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="cs__card-img">
                  ...
                  <div className="cs__card-badges">
                    {comp.conflicto_minerales && (
                      <span className="cs__badge cs__badge--warn" title="Contiene minerales de conflicto">⚠</span>
                    )}
                    {comp.rohs && (
                      <span className="cs__badge cs__badge--ok" title="RoHS compliant">R</span>
                    )}
                  </div>
                </div>
                ...
                <div className="cs__card-body">
                  <p className="cs__card-cat">
                    {CATEGORIAS.find(c => c.id === comp.categoria)?.icon} {comp.subcategoria}
                  </p>
                  <h3 className="cs__card-name">{comp.nombre}</h3>
                  <p className="cs__card-desc">{comp.descripcion.slice(0, 90)}…</p>

                  {/* Métricas rápidas */}
                  <div className="cs__card-metrics">
                    <div className="cs__metric">
                      <span className="cs__metric-label">CO₂</span>
                      <span className="cs__metric-val">{comp.huella_carbono}g</span>
                    </div>
                    <div className="cs__metric">
                      <span className="cs__metric-label">Reciclable</span>
                      <span className="cs__metric-val">{comp.reciclabilidad}%</span>
                    </div>
                    <div className="cs__metric">
