...
import { getSoftwareById, ESTADOS, FASES_RUP, TIPO_DIAGRAMA } from '../models/software.data';
...
const MermaidDiagram = ({ code, id }) => {
  ...
  useEffect(() => {
    ...
    const render = async () => {
      try {
        const { svg } = await window.mermaid.render(`mermaid-${id}`, code.trim());
        if (ref.current) ref.current.innerHTML = svg;
      } catch (e) {
        if (ref.current) ref.current.innerHTML = `<pre style="color:#f87171;font-size:0.75rem">${e.message}</pre>`;
      }
    };
    render();
...
const ESTADO_RF_COLOR = { Implementado: '#16a34a', Pendiente: '#f59e0b', Cancelado: '#dc2626' };

const Badge = ({ text, color }) => (
  <span className="sd__badge" style={{ borderColor: color, color }}>{text}</span>
);
...
export const SoftwareDetail = () => {
  ...
  const tabs = [
    { id: 'demo', label: '▶ Demo', show: !!sw.demo?.youtube_id || !!sw.app?.activa },
    ...
    { id: 'trazabilidad', label: '⊞ Trazabilidad',   show: sw.trazabilidad?.length > 0 },
      ...
      <div className="sd">
        ...
        <div className="sd__hero">
          <div className="sd__hero-left">
            <div className="sd__hero-meta">
              <span className="sd__estado" style={{ color: estado.color }}>
                <span className="sd__dot" style={{ background: estado.color }} />
                {estado.label}
              </span>
              <span className="sd__año">{sw.año}</span>
              <span className="sd__metodologia">RUP · {sw.metodologia?.iteraciones} iteraciones</span>
            </div>
          ...
          <div className="sd__hero-right">
            ...
            <div className="sd__stack-card">
              ...
              {sw.stack?.frontend?.length   > 0 && <div className="sd__stack-row"><span>Frontend</span><div>{sw.stack.frontend.map(t  => <span key={t} className="sd__stack-tag">{t}</span>)}</div></div>}
              {sw.stack?.backend?.length    > 0 && <div className="sd__stack-row"><span>Backend</span><div>{sw.stack.backend.map(t   => <span key={t} className="sd__stack-tag">{t}</span>)}</div></div>}
              {sw.stack?.base_datos?.length > 0 && <div className="sd__stack-row"><span>BD</span><div>{sw.stack.base_datos.map(t => <span key={t} className="sd__stack-tag">{t}</span>)}</div></div>}
              {sw.stack?.otros?.length      > 0 && <div className="sd__stack-row"><span>Otros</span><div>{sw.stack.otros.map(t      => <span key={t} className="sd__stack-tag">{t}</span>)}</div></div>}
            </div>
            ...
            <div className="sd__rol-card">
              ...
              <p className="sd__rol-dur">{sw.metodologia?.duracion} · {sw.metodologia?.equipo} persona(s)</p>
            </div>
          </div>
        </div>
        ...
        <div className="sd__rup">
          {FASES_RUP.map((fase, i) => {
            ...
            return (
              <div key={fase.id} className={`sd__rup-fase ${completada ? 'sd__rup-fase--done' : ''} ${esActual ? 'sd__rup-fase--actual' : ''}`}>
      ...
      <div className="sd__youtube">
        ...
        {tab === 'reqs' && (
          <div className="sd__panel sd__panel--fade">
            ...
            <div className="sd__section">
              ...
              <div className="sd__rf-table">
                <div className="sd__rf-head">
                  <span>ID</span><span>Nombre</span><span>Prioridad</span><span>Estado</span>
                </div>
                {sw.requerimientos.funcionales.map(rf => (
                  <div key={rf.id} className="sd__rf-row">
                    ...
                    <Badge text={rf.prioridad} color={PRIORIDAD_COLOR[rf.prioridad]} />
                    <Badge text={rf.estado}    color={ESTADO_RF_COLOR[rf.estado]} />
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* RNF */}
            <div className="sd__section">
              <h2 className="sd__section-title">Requerimientos No Funcionales</h2>
            ...
            {sw.diagramas[diagActivo] && (
              <div className="sd__uml-viewer">
                <div className="sd__uml-header">
                  <span className="sd__uml-tipo-badge">{TIPO_DIAGRAMA[sw.diagramas[diagActivo].tipo]}</span>
                  <h3>{sw.diagramas[diagActivo].titulo}</h3>
                </div>
                {sw.diagramas[diagActivo].descripcion && (
                  <p className="sd__uml-desc">{sw.diagramas[diagActivo].descripcion}</p>
                )}
                {sw.diagramas[diagActivo].mermaid ? (
                  ...
                  <img src={sw.diagramas[diagActivo].imagen} alt={sw.diagramas[diagActivo].titulo} className="sd__uml-img" />
                ) : (
        ...
        {tab === 'arquitectura' && (
          <div className="sd__panel sd__panel--fade">
            ...
            <div className="sd__section">
              <h2 className="sd__section-title">Patrón Arquitectónico</h2>
              <div className="sd__arch-header">
                <span className="sd__arch-patron">{sw.arquitectura.patron}</span>
              </div>
              <p className="sd__text">{sw.arquitectura.descripcion}</p>
            </div>

            <div className="sd__section">
              <h2 className="sd__section-title">Diagrama de Arquitectura</h2>
              {sw.arquitectura.mermaid ? (
                <MermaidDiagram code={sw.arquitectura.mermaid} id={`arch-${sw.id}`} />
              ) : sw.arquitectura.imagen ? (
                <img src={sw.arquitectura.imagen} alt="Arquitectura" className="sd__uml-img" />
              ) : null}
            </div>
                ...
                <div className="sd__decisions">
                  {sw.arquitectura.decisiones.map((d, i) => (
                    <div key={i} className="sd__decision">
                      <div className="sd__decision-n">{String(i + 1).padStart(2, '0')}</div>
                      <div>
                        <p className="sd__decision-title">{d.decision}</p>
                        <p className="sd__decision-razon">{d.razon}</p>
                        <p className="sd__decision-alt">Alternativas consideradas: {d.alternativas}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {tab === 'patrones' && (
          <div className="sd__panel sd__panel--fade">
            <div className="sd__section">
              <h2 className="sd__section-title">Patrones de Diseño Implementados</h2>
          ...
          <div className="sd__panel sd__panel--fade">
            <div className="sd__section">
              ...
              <p className="sd__text" style={{marginBottom:'1.5rem'}}>
                Vinculación entre requerimientos funcionales, casos de uso y casos de prueba.
              </p>
              <div className="sd__traz-table">
                ...
                {sw.trazabilidad.map((t, i) => {
                  const rf = sw.requerimientos.funcionales.find(r => r.id === t.rf_id);
                  return (
                    <div key={i} className="sd__traz-row">
                      <div>
                        <span className="sd__traz-id">{t.rf_id}</span>
                        {rf && <span className="sd__traz-nombre">{rf.nombre}</span>}
                      </div>
                ...
                {sw.ciclo_vida.hitos.map((h, i) => (
                  <div key={i} className="sd__hito">
                    ...
                    <div className="sd__hito-line">
                      ...
                      {i < sw.ciclo_vida.hitos.length - 1 && <div className="sd__hito-track" />}
                    </div>
            ...
            <div className="sd__section">
              ...
              <div className="sd__fases-detail">
                {sw.metodologia.fases.map((f, i) => {
                  ...
                  return (
                    <div key={i} className={`sd__fase-card ${f.completada ? 'sd__fase-card--done' : ''}`}>
                      <div className="sd__fase-header">
                        <span className="sd__fase-icon">{faseInfo?.icon}</span>
                        <span className="sd__fase-nombre">{faseInfo?.label}</span>
                        {f.completada && <span className="sd__fase-check">✓</span>}
                      </div>
